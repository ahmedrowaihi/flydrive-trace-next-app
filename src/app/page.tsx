  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24">
      ...
      <div className="mb-32 grid text-center lg:mb-0 lg:w-full lg:max-w-5xl lg:grid-cols-4 lg:text-left">
        <form action="/api/fs" method="POST" encType="multipart/form-data">
          <label>Upload to FS</label>
          <input type="file" name="image" placeholder="Upload to FS" />
          <button type="submit">Upload</button>
        </form>
        <form action="/api/s3" method="POST" encType="multipart/form-data">
          <label>Upload to S3</label>
          <input type="file" name="image" placeholder="Upload to S3" />
          <button type="submit">Upload</button>
        </form>
      </div>
    </main>
